...
function getAppStorage(){
  return JSON.parse(localStorage.getItem("tonys-app") || "[]");
}
...
function addSticky(){
  ...
  const noteObject={
    id:Math.floor(Math.random()*100000),    //used to generate the 5 digit number as a whole no
    content:""
  }
...
function saveNotes(notes){
  localStorage.setItem("tonys-app",JSON.stringify(notes));
}
